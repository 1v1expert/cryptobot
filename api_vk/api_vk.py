...
class VkApi(object):
    ...
    def __init__(self, login=None, password=None, token=None, auth_handler=None, captcha_handler=None,
                 config=jconfig.Config, config_filename='vk_config.v2.json', api_version='5.73', app_id=6222115,
                 scope=140492255, client_secret=None):
        """
        ...
        self.http.headers.update({'User-agent': 'Mozilla/5.0 (Windows NT 6.1; rv:52.0) '
                                                'Gecko/20100101 Firefox/52.0'})
        
        self.last_request = 0.0
        
        self.error_handlers = {NEED_VALIDATION_CODE: self.need_validation_handler,
            CAPTCHA_ERROR_CODE: captcha_handler or self.captcha_handler, TOO_MANY_RPS_CODE: self.too_many_rps_handler,
    ...
    @property
    def sid(self):
        return (self.http.cookies.get('remixsid') or self.http.cookies.get('remixsid6'))
    ...
    def auth(self, reauth=False, token_only=False):
        ...
        self.token = self.storage.setdefault('token', {}).setdefault('app' + str(self.app_id), {}).get(
            'scope_' + str(self.scope))
    ...
    def _auth_cookies(self, reauth=False):
        ...
        if not self.check_sid():
            self.logger.info('remixsid from config is not valid: {}'.format(self.sid))
        ...
        if not self.check_token():
            self.logger.info('access_token from config is not valid: {}'.format(self.token))
    ...
    def vk_login(self, captcha_sid=None, captcha_key=None):
        ...
        values = {'act': 'login', 'role': 'al_frame', '_origin': 'https://vk.com', 'utf8': '1', 'email': self.login,
            'pass': self.password, 'lg_h': search_re(RE_LOGIN_HASH, response.text)}
        
        if captcha_sid and captcha_key:
            self.logger.info('Using captcha code: {}: {}'.format(captcha_sid, captcha_key))
            
            values.update({'captcha_sid': captcha_sid, 'captcha_key': captcha_key})
        ...
        if self.sid:
            ...
            raise AuthError('Unknown error. Please send bugreport: https://vk.com/python273')
    ...
    def twofactor(self, auth_response):
        ...
        values = {'act': 'a_authcheck_code', 'al': '1', 'code': code, 'remember': int(remember_device),
            'hash': auth_hash, }
    ...
    def security_check(self, response=None):
        ...
        if code:
            ...
            values = {'act': 'security_check', 'al': '1', 'al_page': '3', 'code': code, 'hash': number_hash, 'to': ''}
    ...
    def api_login(self):
        ...
        response = self.http.get('https://oauth.vk.com/authorize',
            params={'client_id': self.app_id, 'scope': self.scope, 'response_type': 'token'})
        ...
        if 'access_token' in response.url:
            ...
            self.storage.setdefault('token', {}).setdefault('app' + str(self.app_id), {})[
    ...
    def server_auth(self):
        ...
        values = {'client_id': self.app_id, 'client_secret': self.client_secret, 'v': self.api_version,
            'grant_type': 'client_credentials'}
        
        response = self.http.post('https://oauth.vk.com/access_token', values).json()
    ...
    def method(self, method, values=None, captcha_sid=None, captcha_key=None, raw=False):
        ...
        with self.lock:
            ...
            response = self.http.post('https://api.vk.com/method/' + method, values)
        ...
        if 'error' in response:
            ...
            if error.code in self.error_handlers:
                if error.code == CAPTCHA_ERROR_CODE:
                    error = Captcha(self, error.error['captcha_sid'], self.method, (method,),
                        {'values': values, 'raw': raw}, error.error['captcha_img'])
